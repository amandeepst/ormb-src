/*******************************************************************************
 * FileName                   : CardRewardAccountHierarchyGenerator.java
 * Project Name               : WorldPay RMB Implementation
 * Date of Creation           : Mar 24, 2015
 * Version Number             : 0.3
 * Revision History     :
VerNum | ChangeReqNum | Date Modification | Author Name          | Nature of Change
0.1                     Mar 24, 2015        Abhishek Paliwal       Implemented all requirements for CD1.
0.2      NA             Apr 6, 2016         Sunaina                Updated as per Oracle Code review.
0.3      NA             Jun 7, 2017         Ankur/Gaurav                NAP14404 fix
 *******************************************************************************/
...
public class CardRewardAccountHierarchyGenerator extends GenericBusinessObject {


	private List<String> cardRewardCustomerInfo=null;
	private InboundAccountHierarchyLookUps inboundAccountHierarchyLookUps = null;
	
	public CardRewardAccountHierarchyGenerator(InboundAccountHierarchyLookUps inboundAccountHierarchyLookUps){
	...
	/**
	 * @author Venkat
	 * inboundCardRewardAccountHierarchy() method determines Master and Member
	 * Card Reward Accounts and Updates Master Card Reward Account Characteristics
	 * and creates or updates new Card Reward contract with the help of
	 ...
	 * @return
	 */
	public List<String> inboundCardRewardAccountHierarchy(
		...
		//String currencyCd2 = inboundAccountHierarchyId.getCurrencyCd2();
		...
		// *************************************************
		// Determing Master CardReward Account
		// ****************************************************
		...
		if (notNull(preparedStatement.firstRow())) {
			...
			cardRewardCustomerInfo=new ArrayList<String>();
			cardRewardCustomerInfo.add(0,inboundAccountHierarchyId.getTransactionHeaderId());
			cardRewardCustomerInfo.add(1,inboundAccountHierarchyId.getTransactionDetailId());
			cardRewardCustomerInfo.add(2,primaryKey);
			cardRewardCustomerInfo.add(3,hierarchyType);
			cardRewardCustomerInfo.add(4,inboundAccountHierarchyLookUps.getErrorMessageGroup());
			cardRewardCustomerInfo.add(5,messageNumberLocal);
			cardRewardCustomerInfo.add(6,messageNumberLocal);
			cardRewardCustomerInfo.add(7,inboundAccountHierarchyLookUps.getMasterAccountStatus());
			return cardRewardCustomerInfo;
		...
		// ************************************************** 
		// End of Determing Master Card Reward Account
		// ****************************************************

		// ************************************************** 
		// Determing Member Card Reward Account
		// ****************************************************
		...
		if (notNull(preparedStatement.firstRow())) {
			...
			cardRewardCustomerInfo.add(5,messageNumberLocal);
			cardRewardCustomerInfo.add(6,messageNumberLocal);
			cardRewardCustomerInfo.add(7,inboundAccountHierarchyLookUps.getMemberAccountStatus());
			cardRewardCustomerInfo.add(8,"false");
			return cardRewardCustomerInfo;
		}
		...
		// ************************************************ 
		// End of Determing Member Card Reward Account
		// **************************************************
		// ****************************************************** 
		// Member Card Reward AccountUpdate
		// ************************************************************
		...
		if(divisionOfMasterAccount.equals(divisionOfMemberAccount) && currencyCdOfMasterAccount.equals(currencyCdOfMemberAccount) 
			...
			if(notNull(postHierarchyPreparedStatement.firstRow()))
			{
				...
				if (CommonUtils.CheckNull(memberAccountUpdateStatus).trim().startsWith("false")) {
					...
					cardRewardCustomerInfo.add(0,inboundAccountHierarchyId.getTransactionHeaderId());
					cardRewardCustomerInfo.add(1,inboundAccountHierarchyId.getTransactionDetailId());
					cardRewardCustomerInfo.add(2,messageKey);
					cardRewardCustomerInfo.add(3,hierarchyType);
					cardRewardCustomerInfo.add(4,actualErrorMessageCategory);
					cardRewardCustomerInfo.add(5,messageNumberLocal);
					cardRewardCustomerInfo.add(6,actualErrorMessageNumber);
					cardRewardCustomerInfo.add(7,memberAccountUpdateStatus);
					cardRewardCustomerInfo.add(8,"false");
					return cardRewardCustomerInfo;
				}
			}
			...
			// *************************************************** 
			// End of Member Card Reward AccountUpdate
			// ******************************************************

			// ****************************************************** 
			// Master Card Reward AccountUpdate
			// ************************************************************
			...
			if(accountCharacteristicPreparedStatement.list().size() == 0)
			{
				...
				if (CommonUtils.CheckNull(masterAccountUpdateStatus).trim().startsWith("false")) {
					...
					cardRewardCustomerInfo.add(5,messageNumberLocal);
					cardRewardCustomerInfo.add(6,actualErrorMessageNumber);
					cardRewardCustomerInfo.add(7,masterAccountUpdateStatus);
					cardRewardCustomerInfo.add(8,"false");
					return cardRewardCustomerInfo;
				}
			}
			...
			// *************************************************** 
			// End of Master Card Reward AccountUpdate
			// ******************************************************
			...
			if (notNull(preparedStatement.firstRow())) {
				...
				cardRewardCustomerInfo.add(5,messageNumberLocal);
				cardRewardCustomerInfo.add(6,messageNumberLocal);
				cardRewardCustomerInfo.add(7,inboundAccountHierarchyLookUps.getMasterMasterAssociationStatus());
				cardRewardCustomerInfo.add(8,"false");
				return cardRewardCustomerInfo;
			...
			// ***************************************************
			// Card Reward Account Hierarchy for Card Reward Contract
			// ******************************************************

			String cardRewardAccountHierarchyStatus = inboundAccountHierarchyHelper
					.createAccountHierarchy(
							inboundAccountHierarchyId,
							masterAccountId,
							memberAccountId,
							hierarchyType,
							inboundAccountHierarchyLookUps.getCardRewardContractType());
			String serviceAgreementId = inboundAccountHierarchyHelper.serviceAgreementId;
			if (CommonUtils.CheckNull(cardRewardAccountHierarchyStatus).trim()
					.startsWith("false")) {
				String Message = cardRewardAccountHierarchyStatus.substring(
						cardRewardAccountHierarchyStatus.indexOf("~") + 1,
						cardRewardAccountHierarchyStatus.lastIndexOf("~") - 1);
				String actualErrorMessageCategory = Message.substring(
						Message.indexOf("~") + 1).trim();
				String actualErrorMessageNumber = cardRewardAccountHierarchyStatus
						.substring(
								cardRewardAccountHierarchyStatus.lastIndexOf("~") + 1,
								cardRewardAccountHierarchyStatus.length()).trim();
				...
				cardRewardCustomerInfo.add(1,inboundAccountHierarchyId.getTransactionDetailId());
				cardRewardCustomerInfo.add(2,messageKey);
				cardRewardCustomerInfo.add(3,inboundAccountHierarchyLookUps.getCardRewardContractType());
				cardRewardCustomerInfo.add(4,actualErrorMessageCategory);
				cardRewardCustomerInfo.add(5,messageNumberLocal);
				cardRewardCustomerInfo.add(6,actualErrorMessageNumber);
				cardRewardCustomerInfo.add(7,cardRewardAccountHierarchyStatus);
				cardRewardCustomerInfo.add(8,"false");
				return cardRewardCustomerInfo;
			}
		}
		...
		if(!divisionOfMasterAccount.equals(divisionOfMemberAccount)) {
			...
			cardRewardCustomerInfo.add(5,messageNumberLocal);
			cardRewardCustomerInfo.add(6,messageNumberLocal);
			cardRewardCustomerInfo.add(7,inboundAccountHierarchyLookUps.getDivisionOfAccountsStatus());
			cardRewardCustomerInfo.add(8,"false");
			return cardRewardCustomerInfo;
		}
		...
		if(!currencyCdOfMasterAccount.equals(currencyCdOfMemberAccount)) {
			...
			cardRewardCustomerInfo.add(5,messageNumberLocal);
			cardRewardCustomerInfo.add(6,messageNumberLocal);
			cardRewardCustomerInfo.add(7,inboundAccountHierarchyLookUps.getCurrencyCodesOfAccountsStatus());
			cardRewardCustomerInfo.add(8,"false");
			return cardRewardCustomerInfo;
		}
		...
		if(!currencyCd.equals(currencyCdOfMasterAccount)) {
			...
			cardRewardCustomerInfo.add(5,messageNumberLocal);
			cardRewardCustomerInfo.add(6,messageNumberLocal);
			cardRewardCustomerInfo.add(7,inboundAccountHierarchyLookUps.getCurrencyCodesOfMasterPersonAndMasterAccountStatus());
			cardRewardCustomerInfo.add(8,"false");
			return cardRewardCustomerInfo;
		}
		//		ERROR IERROR IS THROWN IF CURRENCY CODES ARE DIFFERENT FOR CHILD PERSON AND MEMBER ACCOUNTS
		// ********************************************************** 
		// End of Charging Account Hierarchy for Card Reward Contract
		// *************************************************************


		cardRewardCustomerInfo=new ArrayList<String>();
		cardRewardCustomerInfo.add(0,null);
		cardRewardCustomerInfo.add(1,null);
		cardRewardCustomerInfo.add(2,null);
		cardRewardCustomerInfo.add(3,null);
		cardRewardCustomerInfo.add(4,null);
		cardRewardCustomerInfo.add(5,null);
		cardRewardCustomerInfo.add(6,null);
		cardRewardCustomerInfo.add(7,null);
		cardRewardCustomerInfo.add(8,"true");
		return cardRewardCustomerInfo;
	}

}// End of Card Reward AccountHierarchyGenerator Class
