 ...
 *            , @AlgorithmSoftParameter (name = rcSequence2, required = true, type = integer)
 *            , @AlgorithmSoftParameter (name = roundToDecimal, required = true, type = integer)})
 */
public class CmAddRateCompVal_Impl extends CmAddRateCompVal_Gen implements
	...
	public void invoke() {
		...
		if(notNull(billSegmentCalculationHeaderDataList) && !billSegmentCalculationHeaderDataList.isEmpty()){
			...
			if(calcLines!=null && calcLines.size()>0) {
				...
				for(BillCalculationLineData calcLine:  calcLines) {
					...
					if(calcLineSeq.compareTo(getRcSequence2())==0){
